...
- [~] Step 5 — Verification pass (guest/free/Plus, empty states, mobile, `pnpm check`). Partial: added `e2e/review.test.ts` (2026-09-03) covering the free/guest path — `/review`'s empty state, the picker, a full due-card render through `/api/review-entries`, and both `/stats` CTAs (4a global banner shown/hidden, 4b per-level link). Plus's path (Supabase-backed progress + the per-category due badges in `LevelStatRows.svelte`) isn't covered, for the same reason two existing tests in `e2e/flashcard.test.ts` are `.fixme` — no seeded Plus test account with real `card_progress` rows exists in this suite. **I haven't run these** — no command-execution access to this repo, only file read/write — so `pnpm lint` and `pnpm test:e2e` still need a real run before trusting this box checked. Mobile pass and `pnpm check` also still outstanding.
