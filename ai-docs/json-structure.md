...
**Statistics and CEFR estimates are lemma-based.** Counting inflected forms inflates word counts artificially. Norwegian nouns inflect for definiteness (_hus_ / _huset_), verbs take several forms (_å lese_, _leser_, _leste_, _lest_), and adjectives agree with gender and number. A learner who has studied 80 lemmas should not be credited with 300+ "known words" due to inflection. The CEFR estimate on the stats page should count unique lemmas reviewed, not unique `norsk` strings.
...
- **Deduplication across categories** — the same lemma appearing in two categories (e.g. _å lese_ in both `media` and `education`) is counted once
- **Accurate CEFR estimates** — "you know 73% of A2 vocabulary" means 73% of unique A2 lemmas, not 73% of flashcard tokens
- **Word family grouping** (optional, later) — _reise_, _reiser_, _reisende_ all share the lemma `reise`
