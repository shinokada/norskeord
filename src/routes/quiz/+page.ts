...
export const load: PageLoad = async ({ url, parent }) => {
  ...
  // Validate level param against known loaders to prevent silent empty pools.
  // Fall back to the user's profile target_level when no URL param is given.
  const profileLevel = typeof targetLevel === 'string' ? targetLevel.toLowerCase() : null;
  const rawLevel = levelParam ?? profileLevel;
  const validLevel = rawLevel && rawLevel in vocabLoaders ? rawLevel : null;

  // Load all levels in parallel for the distractor pool.
