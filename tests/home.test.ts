...
test('index page has expected meta description', async ({ page }) => {
  ...
  await expect(metaDescription).toHaveAttribute('content', 'Master Norwegian Vocabulary with Flashcard.');
});

test('index page has expected meta keywords', async ({ page }) => {
  const metaKeywords = page.locator('meta[name="keywords"]');
  await expect(metaKeywords).toHaveAttribute('content', 'Norwegian, language game, flashcards, learning, language');
});
...
test('index page has expected meta og', async ({ page }) => {
  ...
  await expect(metaOgDescription).toHaveAttribute('content', 'Master Norwegian Vocabulary with Flashcard.');
...
test('index page has expected meta twitter', async ({ page }) => {
  ...
  await expect(metaTwitterDescription).toHaveAttribute('content', 'Master Norwegian Vocabulary with Flashcard.');
