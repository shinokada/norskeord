#!/usr/bin/env node
/**
 * check-b1-grammar-vocab.mjs
 *
 * Per ai-docs/implementation/b1-grammar.md ("Vocab verification script" +
 * Phase 3). Every B1 grammar question added by that plan should contain at
 * least one real vocab-b1.json / uttrykk-b1.json headword (or, failing
 * that, a vocab-a1.json / vocab-a2.json / uttrykk-a1.json / uttrykk-a2.json
 * one, since B1 content naturally reuses a lot of earlier-level vocabulary),
 * so the grammar content stays anchored to vocabulary the learner is
 * actually studying at that level. Adapted from
 * scripts/check-a2-grammar-vocab.mjs — same matching logic, pointed at the
 * 32 B1 topic-touches instead of the 16 A2 ones.
 *
 * What it does:
 *   1. Loads src/lib/data/grammar.json, filters to the 32 B1 topic-touches
 *      (B1_TOPICS below — 9 new topics + 23 reused topics), restricted to
 *      cefr: 'B1' only, since the reused topics also carry A1/A2/B2/C
 *      entries that aren't in scope here.
 *   2. Loads all `lemma` values from vocab-b1.json + uttrykk-b1.json, and
 *      vocab-a1.json + vocab-a2.json + uttrykk-a1.json + uttrykk-a2.json as
 *      a fallback pool.
 *   3. For each B1 question, concatenates its text fields and checks for at
 ...
 *
 * Usage:
 *   node scripts/check-b1-grammar-vocab.mjs                  # check all 32 B1 topic-touches
 *   node scripts/check-b1-grammar-vocab.mjs passiv-bli-s da-naar
 *                                                             # check specific topic(s)
 *   node scripts/check-b1-grammar-vocab.mjs --strict          # exit 1 if any unmatched
 */
...
// The 32 B1 topic-touches from ai-docs/implementation/b1-grammar.md:
// 9 new topics + 23 reused topics (which also carry non-B1 entries elsewhere
// in grammar.json — those are excluded below via the cefr === 'B1' filter).
const B1_TOPICS = new Set([
  // 9 new topics
  'framtid-uttrykk',
  'for-sa-arsak-folge',
  'da-naar',
  ...
  // 23 reused topics (B1-cefr entries only)
  'presens-perfektum',
  'og-men',
...
// Reused topics also carry A1/A2/B2/C entries — restrict to cefr: 'B1' so
// this script only checks the entries this plan actually added.
const questions = grammar.filter((q) => B1_TOPICS.has(q.topic) && q.cefr === 'B1');
const targetQuestions =
  topicFilter.length > 0 ? questions.filter((q) => topicFilter.includes(q.topic)) : questions;

if (targetQuestions.length === 0) {
  console.log('No matching B1-topic questions found in grammar.json.');
  console.log(
...
// Build the lemma list once. B1 + uttrykk-b1 first, then A1/A2 + their
// uttrykk files as a fallback pool (B1 content leans on earlier-level
...
console.log(
  `🔍  Checking ${targetQuestions.length} B1 grammar question(s) against ${singleWordLemmas.size} single-word + ${phraseLemmas.length} phrase lemmas`
);
console.log(`${'═'.repeat(60)}`);
...
for (const [topic, qs] of byTopic) {
  ...
  for (const q of unmatched) {
    console.log(`    ❌  ${q.id}: no vocab-b1/a1/a2.json / uttrykk-b1/a1/a2.json headword found`);
    console.log(
