...
console.log('Top-level shape:', Array.isArray(data) ? `array of ${data.length}` : Object.keys(data));

// Search the whole file text for causative-related keywords
const keywords = ['causative', 'kausativ', 'få noen til', 'til å', 'få + '];
for (const kw of keywords) {
  const count = (raw.match(new RegExp(kw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi')) || []).length;
...
if (Array.isArray(data)) {
  for (const item of data) {
    ...
    if (text.includes('få') && (text.includes('til å') || text.includes('causat') || text.includes('kausativ'))) {
      console.log('\n--- possible match ---');
