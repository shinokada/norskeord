...
console.log(`${write ? 'APPLYING' : 'DRY RUN'} — ${toRemove.length} deletion(s) from uttrykk-${level}.json:\n`);
for (const { decision, entry } of toRemove) {
	console.log(`  - remove ${entry.id}  "${entry.norsk}"  (${decision.bucket ?? decision.type}: ${decision.reason ?? ''})`);
}
...
if (verify.length !== after.length || stillPresent.length > 0) {
	console.error('VERIFICATION FAILED — disk state does not match intended write. Investigate before continuing.');
	process.exit(1);
}
console.log(`Verified via fresh read-back: ${verify.length} entries, all ${removeIds.size} targets confirmed absent.`);
