...
if (decisions.length === 0) {
	console.log(`No decisions logged yet for level ${level} (scripts/reclass/decisions/${level}.jsonl missing or empty).`);
	process.exit(0);
}
...
for (const d of decisions) {
	...
	const sourceGone =
		d.type === 'add_vocab' ? true : !findEntry(uttrykkList, d.source || {});
	...
	if (d.type === 'delete') {
		...
		else if (sourceGone && vocabState === 'missing') state = 'PARTIAL (source deleted, vocab never added)';
		else if (!sourceGone && vocabState === 'present') state = 'PARTIAL (vocab added, source never deleted)';
		else state = 'pending';
