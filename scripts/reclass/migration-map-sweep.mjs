...
for (const { key, id, decision } of toApply) {
  const desc = decision.action === 'remap' ? `remap to ${decision.to}` : `delete key`;
  console.log(
    `  ${write ? 'APPLYING' : 'PLANNED'}  ${key} -> ${id}  :: ${desc}  (${decision.reason ?? ''})`
  );
}
...
for (const { key, decision } of toApply) {
  if (decision.action === 'remap') {
    if (!validIds.has(decision.to)) {
      console.error(
        `Refusing: remap target ${decision.to} for ${key} does not exist in production either.`
      );
      process.exit(1);
    }
    map[key] = decision.to;
