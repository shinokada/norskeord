...
const line = decisions.find((d) => (d.vocab?.norsk || d.source?.norsk || '').toLowerCase().includes('bunns'));
...
console.log(JSON.stringify(uttrykkA2.filter(e => (e.norsk||'').includes('bunns') || (e.lemma||'').includes('bunns')), null, 2));
console.log('\n### vocab-a2.json entries matching "bunns"');
console.log(JSON.stringify(vocabA2.filter(e => (e.norsk||'').includes('bunns') || (e.lemma||'').includes('bunns')), null, 2));
